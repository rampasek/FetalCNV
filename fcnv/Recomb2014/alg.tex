...
Additionally, for each SNP position we multiply transitions by the copy number priors obtained by the method above. Specifically, each edge incoming to a state is multiplied by the corresponding prior of inheriting that many haplotypes. %the transition probabilities  \ref{fig:hmm_main:a}

\todo[inline]{argue about default transition probs}
