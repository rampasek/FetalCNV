...
do
	...
	do
		...
		do
			qsub -q all.q -V -R y -l h_vmem=5G -l h_rt=15:00:00 -S /bin/bash $exec_path/sim_deletion.sh $chr $begin $length $haplo $data_path $plasma_path $exec_path
			echo $begin' '$length' '$haplo

			for src in $src_set
			do
				qsub -q all.q -V -R y -l h_vmem=5G -l h_rt=15:00:00 -S /bin/bash $exec_path/sim_duplicate.sh $chr $begin $length $src $haplo $data_path $plasma_path $exec_path
...
do
    ...
    qsub -q all.q -V -R y -pe parallel 7 -l h_vmem=5G -l h_rt=05:00:00 -o $log_file -e $log_file -S /usr/bin/pypy $exec_path/prepare_fcnv_input.py $data_path/mp.phase.vcf $bam_file $data_path/__M.part.bam $data_path/__P.part.bam $data_path/regions.bed /dupa-filer/laci/centromeres $results_path
