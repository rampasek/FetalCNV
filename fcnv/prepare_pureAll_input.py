...
def main():
    #parse ARGs
    parser = argparse.ArgumentParser(description='Prepare SNP support data for FCNV without paternal info, using all 1kGP SNP positions. Read filenames: for joined M&P phased .vcf file; plasma, and M .bam files; and for centromeres list.')
    parser.add_argument('filenames', type=str, nargs='+', help='paths to 1) .vcf file with phased M SNPs; 2) reads in SAM format for plasma, M, and P(just dummy) samples; 3) valid regions BED file 4) centromeres list file; 5) result path.')
...
#        if not (wbegin <= pos <= wend): continue
    ...
    #cdir = os.getcwd() + '/'
    pos_file_name = res_path + '/../1kGPpos/' + processed_chr + '.all1kGPpos.txt'
    plasma_id = plasma_id.replace('part', processed_chr)
    tmp_vcf_prefix = tmp_dir + '/__tmp' + plasma_id
    pile_prefix = res_path + '/' + plasma_id
    cmd = "span_samtools.sh /filer/hg19/hg19.fa {0} {1} {2} {3} {4} {5} {6} {7} {8}".format(pos_file_name, \
        args.filenames[PLR], args.filenames[MR], '/dev/null', tmp_vcf_prefix, args.filenames[BED], 0, processed_chr, pile_prefix)
    os.system(cmd)
    ...
    for R in [PLR, MR]:
        ...
        while True:
            ...
            if pos in loci:
                ref = loci[pos][2][0]
                alt = loci[pos][2][1]
            else: #not in M .vcf file but in 1kGP SNP pos => a homozygous ref. position
                ref = line[3][0]
                alt = line[4][0]
